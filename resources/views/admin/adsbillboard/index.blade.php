...
@section('title','لیست تبلیغات بیلبرد')
@section('part','لیست تبلیغات بیلبرد')
@section('content')
    <div class="row">
        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    ...
                    {!! Form::open(array('action' => array('Admin\AdsBillboardController@postDelete'),'style'=>'float: left')) !!}
                    <div class="box-tools">
                        <a href="#" data-toggle="modal" data-target="#search" class="btn btn-info btn-xs">
                            <i class="fa fa-search"></i> جستجو
                        </a>

                        {{--<a class="btn btn-success btn-xs" href="{{URL::action('Admin\AdsBillboardController@getAdd')}}"
                           data-toggle="tooltip"
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach($data as $row)
                            <tr>
                                ...
                                <td>{{$row->title}}</td>
                                <td>{{@$row->billboard->location}}</td>

                                <td>{{@$row->billboard->type}}</td>
                                <td>
                                    @if(file_exists('assets/uploads/ads_video/medium/'.$row->image))
                                        <img src="{!! asset('assets/uploads/ads_video/medium/'.$row->image) !!}"
                                             class="img-rounded"
                                ...
                                <td>
                                    <center>
                                        <a href="{{URL::action('Admin\AdsBillboardController@getEdit',$row->id)}}"
                                           data-toggle="tooltip"
