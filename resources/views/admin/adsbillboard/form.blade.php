        ...
        <div class="row">
            <div class="col-md-6">
                ...
                {!! Form::file('image',array(
                    'class'=>'form-control')) !!}
                @if(isset($data))
                    @if(file_exists('assets/uploads/ads_billboard/medium/'.$data->image))
                        <img src="{!! asset('assets/uploads/ads_billboard/medium/'.$data->image) !!}"
                             class="img-rounded"
            ...
            <div class="col-md-6">
                ...
                {!! Form::file('video',array(
                    'class'=>'form-control')) !!}
                @if(isset($data))
                    @if(file_exists('assets/uploads/ads_billboard/'.$data->video))
                        <?php $ext = pathinfo($data->video, PATHINFO_EXTENSION); ?>
                        <video width = "250" height = "150" controls>
                            <source src = "{{App::make('url')->to('/')}}/assets/uploads/ads_billboard/{{$data->video}}" type = "video/{{$ext}}">
                        </video>
                    @else
                        <img src="{!! asset('assets/uploads/notFound.jpg') !!}"
                             class="img-rounded"
