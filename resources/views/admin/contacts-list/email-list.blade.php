...
@section('title','ارسال ایمیل به کاربران معرفی شده')
@section('part','ارسال ایمیل به کاربران  معرفی شده')
@section('content')
	<div class="row">
		...
		<div class="col-xs-12">
		  <div class="box">
			<div class="box-header">
			  ...
			  {!! Form::open(array('action' => array('Admin\ContactsListController@postDelete'),'style'=>'float: left')) !!}
			  <div class="box-tools">
				...
				<a class="btn btn-success btn-xs" href="{{URL::action('Admin\ContactsListController@getAdd')}}" data-toggle="tooltip"
			  ...
			  <table class="table table-hover">
				...
				@foreach($data as $row)
					...
					<tr>
						...
						<td>{{$row->sender}}</td>
						<td>{{$row->count}}</td>
						<td>{{count($row->emailUser)}}</td>
						<td>{{count($row->emailUserSuccess)}}</td>
						<td>{{count($row->emailUserFaild)}}</td>
						<td>
						@php
							if(count($row->emailUser) == 0)
								$persent = 0;
							else
								$persent = (count($row->emailUserSuccess)*100)/count($row->emailUser);
						@endphp
						<span>
							{{round($persent)}} درصد 
						</span>
							<div class="progress progress-sm active">
								<div class="progress-bar @if($persent == 100) progress-bar-success @else progress-bar-warning @endif progress-bar-striped" role="progressbar" 
									aria-valuenow="{{count($row->emailUserSuccess)}}" aria-valuemin="0" aria-valuemax="{{count($row->emailUser)}}" 
									style="width: {{$persent}}%">
						...
						<td>
							<center>
							@php
								if(count($row->emailUser) == count($row->emailUserSuccess) ){
							...
							@endphp
								<a data-toggle="tooltip" data-original-title="{{ $msg }}" 
									href="@if(!$success){{URL::action('Admin\ContactsListController@getSetting',[$row->id])}}@else#@endif"
									   class="btn btn-{{ ($row->status) ? 'success' : 'danger' }} btn-xs"><span
											class="fa fa-{{ ($row->status) ? 'check' : 'times' }}"></span>
											{{ $msg }}
									</a>
									
									<a href="{{URL::action('Admin\ContactsListController@getEdit',[$row->id])}}" data-toggle="tooltip"
