...
@section('title','اتصالات کاربر')
@section('part','اتصالات '.$user->name.' '.$user->family)
@section('content')
	<div class="row">
			  ...
			  <table class="table table-hover">
				...
				@foreach($data as $row)
					...
					<tr>
						...
						<td>
							<center>
								<a href="{{URL::action('Admin\NetworkController@getRemoveFriendRequest',[$user->id,$row->id])}}" data-toggle="tooltip"
									onclick="return confirm('آیا از حذف اتصال مطمئن هستید.');"
								   data-original-title="حذف اتصال" class="btn btn-danger  btn-xs" id="edit{{$row->id}}"><i
											class="fa fa-trash"></i> حذف اتصال </a>
							</center>
						</td>
					</tr>
