...
@section('title','درخواست ها ارسال شده به هسته علمی')
@section('part','درخواست ها ارسال شده به هسته علمی')
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box box-primary">
								...
								@foreach($data->reply as $item) 
									@if($item->admin_id != '-1')
									 ...
									 @else
										...
										<li>
										<div class="message-data align-left">
										  <span class="message-data-name"><i class="fa fa-circle you"></i> {{@$item->user->name.' '.@$item->user->family}}  - {{jdate('Y/m/d H:i',$item->created_at->timestamp)}}</span>
										</div>
										<div class="message you-message" style="text-align: right;direction: rtl;">
											{!! $item->content !!}
										</div>
                ...
                {!! Form::open(array('action' => array('Admin\MessageCoreController@postView',$data->id),'role' => 'form','id' => 'ejavan_form')) !!}
