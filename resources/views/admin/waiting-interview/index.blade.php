...
@section('title','اعضا در انتظار مصاحبه')
@section('part','اعضا در انتظار مصاحبه')
@section('content')
	<div class="row">
		...
		<div class="col-xs-12">
		  <div class="box">
			...
			@if(Auth::user()->hasPermission('waiting-interview.delete'))
				{!! Form::open(array('action' => array('Admin\WaitingInterviewController@postDelete'),'style'=>'float: left')) !!}
			@endif
			  <div class="box-tools">
				<a href="#" data-toggle="modal" data-target="#search"  class="btn btn-info btn-xs">
					<i class="fa fa-search"></i> جستجو
				</a>
				@if(Auth::user()->hasPermission('waiting-interview.add'))
					<a class="btn btn-success btn-xs" href="{{URL::action('Admin\WaitingInterviewController@getAdd')}}" data-toggle="tooltip"
				...
				@foreach($data as $row)
					...
					<tr @if($row->mobile == null) class="danger" @endif>
						...
						<td>
							...
							@if(Auth::user()->hasPermission('waiting-interview.edit'))
								<a href="{{URL::action('Admin\WaitingInterviewController@getEdit',[$row->id])}}" data-toggle="tooltip"
							...
							@endif	
								<a style="cursor:pointer" data-toggle="tooltip"
								   ...
								   onclick="caller('{{URL::action('Admin\WaitingInterviewController@getCaller',$row->id)}}',{{$row->id}})" ><i
											class="fa fa-phone"></i> 
