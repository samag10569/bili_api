			  ...
			  <table class="table table-hover">
				...
				@foreach($data as $row)
					<tr @if($row->status) class="success" @else class="danger" @endif>
						<td>{{$row->id}}</td>
						<td>{{@$row->user->name.' '.@$row->user->family.' - '.$row->user_id}}</td>
						...
						<td>
							<center>
								<a href="{{URL::action('Admin\OrdersController@getFactor',[$row->id])}}" data-toggle="tooltip"
								   data-original-title="مشاهده فاکتور" class="btn btn-warning  btn-xs"><i
											class="fa fa-edit"></i> مشاهده فاکتور </a>
							</center>
						</td>
					</tr>
