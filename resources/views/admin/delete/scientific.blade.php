...
@section('title','مطالب علمی مدیران')
@section('part','مطالب علمی مدیران')
@section('content')
    <div class="row">
        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    ...
                    {!! Form::open(array('action' => array('Admin\DeleteController@postScientific'),'style'=>'float: left')) !!}
                    <div class="box-tools">
                        ...
                        <button type="submit" onclick="return confirm('آیا از حذف موارد مطمئن هستید.');"
                                data-toggle="tooltip"
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach($data as $row)
                            ...
                            <tr>
                                ...
                                <td>{{$row->title}}</td>
                                <td>{{str_limit(strip_tags($row->content_short),40)}}</td>
                                <td>
                                ...
                                <td>
                                    @if($row->user_id != 0 and $row->user_id != null)
                                    {{$row->user->name}} {{@$row->user->family }}</td>
                                @else
                                @endif
                                <td>
                                    @if(file_exists(public_path('assets/uploads/scientific/medium/'.$row->image)))
                                        <img src="{!! asset('assets/uploads/scientific/medium/'.$row->image) !!}"
                                             class="img-rounded"
                                ...
                                <td>
                                    <center>
                                        <a href="{{URL::action('Admin\DeleteController@getCancelScientific',$row->id)}}" data-toggle="tooltip"
                                           data-original-title="لغو " class="btn btn-warning  btn-xs">
                                            لغو
                    ...
                    <div class="widget flat radius-bordered">
                        <div class="widget-body">
                            <div id="registration-form">
                                ...
                                <div class="form-group">
                                    {!! Form::label('category_id','مجموعه',array('class'=>'col-lg-3 control-label')) !!}
                                    <div class="col-lg-9">
