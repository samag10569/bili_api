		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header">
					...
					{!! Form::open(array('action' => array('Admin\NewsletterSmsController@postDelete'),'style'=>'float: left')) !!}
					<div class="box-tools">
						<a href="#" data-toggle="modal" data-target="#search"  class="btn btn-info btn-xs">
							<i class="fa fa-search"></i> جستجو
						</a>

						<a class="btn btn-success btn-xs" href="{{URL::action('Admin\NewsletterSmsController@getAdd')}}" data-toggle="tooltip"
					...
					<table class="table table-hover">
						...
						@foreach($data as $row)
							...
							<tr>
								...
								<td>
									@php
										$persent = (count($row->phoneUserSuccess)*100)/count($row->phoneUser);
									@endphp
									<span>
							{{$persent}} درصد 
						</span>
									<div class="progress progress-sm active">
										<div class="progress-bar @if($persent == 100) progress-bar-success @else progress-bar-warning @endif progress-bar-striped" role="progressbar"
											 aria-valuenow="{{count($row->phoneUserSuccess)}}" aria-valuemin="0" aria-valuemax="{{count($row->phoneUser)}}"
											 style="width: {{$persent}}%">
