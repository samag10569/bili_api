...
@section('title','پروژه های موظفی')
@section('part','پروژه های موظفی')
@section('content')
    <div class="row">
                ...
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                        ...
                        @foreach($data as $row)
                            ...
                            <tr @if($row->status_id == 4) class="success"
                                @elseif($row->status_id == 3) class="warning" @endif>
                                <td>{{$row->name.' '.$row->family}}</td>
                                <td>{{$row->user_code}}</td>
                                <td>
                                    {{@$row->supervisorInfo->name.' '.@$row->supervisorInfo->family}}
                                </td>
                                <td>{{@$row->projectRequiredStatus->title}}</td>
                                <td>{{$row->title}}</td>
                                <td>
                                    <center>
                                        <a href="{{URL::action('Admin\ProjectRequiredController@getEdit',[$row->id])}}"
                                           data-toggle="tooltip"
                                           data-original-title="ویرایش اطلاعات" class="btn btn-warning  btn-xs"
                                           id="edit{{$row->id}}"><i
                                                    class="fa fa-edit"></i> مشاهده پروژه موظفی </a>
                                    </center>
                                </td>
                            </tr>
                        @endforeach
