...
class AdsImageController extends Controller
{
    public function getIndex(Request $request)
    {
        ...
        if (!$request->has('sort')) {
            $query->orderBy('ads_images.id', 'DESC');
        }

        $data = $query->paginate(15);

        return View('admin.adsimage.index')
            ->with('data', $data);
    }
    ...
    public function getAdd()
    {
        ...
        return view('admin.adsimage.add')->with('status', $status)->with('gender', $gender)
            ->with('shakhe', $shakhe)
    ...
    public function postAdd(AdsImageRequest $request)
    {
        $input = $request->all();
        ...
        return Redirect::action('Admin\AdsImageController@getIndex')
    ...
    public function getEdit($id)
    {
        ...
        $cities = City::pluck('title', 'id')->all();
        $cities = ['' => 'انتخاب کنید . . .'] + $cities;
        return View('admin.adsimage.edit')
    ...
    public function postEdit($id, AdsImageRequest $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            File::delete($pathBig . $adsimage->image);
            File::delete($pathMedium . $adsimage->image);
            if ($fileName) {
                $input['image'] = $fileName;
            } else {
                return Redirect::back()->with('error', 'عکس ارسالی صحیح نیست.');
        ...
        $adsimage->where('id', $id)->update($input);


        return Redirect::action('Admin\AdsImageController@getIndex')->with('success', 'آیتم مورد نظر با موفقیت ویرایش شد.');

    }



    public function postDelete(AdsImageRequest $request)
    {

        if (AdsImage::destroy($request->get('deleteId'))) {
            return Redirect::action('Admin\AdsImageController@getIndex')
