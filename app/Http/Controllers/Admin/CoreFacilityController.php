...
use App\Http\Requests\CoreFacilityRequest;
...
class CoreFacilityController extends Controller
{
    public function getIndex(Request $request)
    {
        ...
        return View('admin.core-facility.index')
    ...
    public function getEdit($id)
    {
        ...
        return View('admin.core-facility.edit')
            ->with('state_id', $state_id)
    ...
    public function postEdit($user_id, CoreFacilityRequest $request)
    {
        $rules = [
        ...
        if ($validator->passes()) {
            ...
            if ($request->hasFile('image')) {
                ...
                $fileName = $uploader->uploadPic($request->file('image'), 'assets/uploads/member/');
                $pathBig = 'assets/uploads/member/big/';
                $pathMedium = 'assets/uploads/member/medium/';
            ...
            return Redirect::action('Admin\CoreFacilityController@getIndex')
    ...
    public function postDelete(CoreFacilityRequest $request)
    {
    ...
    {
        $user = User::select('id', 'name', 'family', 'user_code', 'core_scientific', 'station')->find($user_id);
        $allotment_data = [];
        ...
        $groupsId = [];
        foreach ($user->factualyList as $item) {
            $groupsId[] = $item->id;
        }

        $allotment_id = Allotment::orderby('title', 'ASC')->pluck('title', 'id')->all();
        $allotment_id = ['' => 'انتخاب کنید'] + $allotment_id;

        $messages = AllotmentMessage::whereUserId($user_id)->with('userAdmin')->get();

        return View('admin.core-facility.allotment')
    ...
    {
        $checker = new UserCheck();
        $data = $checker->getDeleteMessage($message_id);
        event(new LogUserEvent($message_id, 'delete', Auth::user()->id));
        return json_encode($data);
    }

    public function getEditMessage($message_id, Request $request)
    {
        $message = AllotmentMessage::find($message_id);
        $data['status'] = 'no';
        $data['msg'] = 'مشکل در ویرایش اطلاعات، لطفا مجدد تلاش نمایید.';
        $message->content = $request->get('content');
        if ($message->save()) {
            $data['status'] = 'ok';
            $data['msg'] = 'اطلاعات با موفقیت ویرایش شد.';
        }
        event(new LogUserEvent($message_id, 'edit', Auth::user()->id));
        return json_encode($data);
    }

    public function getAddMessage(Request $request)
    {
        $data['status'] = 'no';
        $data['msg'] = 'مشکل در ویرایش اطلاعات، لطفا مجدد تلاش نمایید.';
        $message = new AllotmentMessage();
        $message->user_id = $request->get('user_id');
        $message->content = $request->get('content');
        $message->allotment_id = $request->get('allotment_id');
        $message->admin_id = Auth::user()->id;
        if ($message->save()) {
            $data['status'] = 'ok';
            $data['msg'] = 'اطلاعات با موفقیت ویرایش شد.';
        }
        event(new LogUserEvent($message->id, 'add', Auth::user()->id));
        return json_encode($data);
    }

    public function postCoreScientific(CoreFacilityRequest $request, $user_id)
    {
        $user = User::find($user_id);
        $user->update([
            'core_scientific' => $request->get('core_scientific'),
            'station' => $request->get('station'),
        ]);
        if ($request->get('core_scientific')) {
